...
builder.Services.AddMassTransit(busConfigurator =>
{
    busConfigurator.AddConsumer<TaskAssignedEventConsumer>()
     ...
     {
         endpointConfigurator.Name = "notification-task-assigned";
     });

    busConfigurator.AddConsumer<TaskCreatedEventConsumer>()
        .Endpoint(endpointConfigurator =>
        {
            endpointConfigurator.Name = "notification-task-created";
        });

    busConfigurator.AddConsumer<MemberAddedEventConsumer>()
        .Endpoint(endpointConfigurator =>
        {
            endpointConfigurator.Name = "notification-member-added";
        });

    busConfigurator.AddConsumer<ProjectCreatedEventConsumer>()
        .Endpoint(endpointConfigurator =>
        {
            endpointConfigurator.Name = "notification-project-created";
        });
...
if (app.Environment.IsDevelopment())
{
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "Flowist NotificationService API v1");
